...
namespace WindowsFormsApp3
{
    public partial class UC_NhaCungCap : UserControl
    {
        ...
        void LoadData()
        {
            dgvNCC.DataSource = BUS_NhaCungCap.Intance.getListNCC();
        ...
        {
            txtMaNCC.Text = BUS_NhaCungCap.Intance.loadMaNCC();
            if (check == true)
            {
                check = !check;
            ...
            else
            {
                if (KiemTraNhap())
                {
                    ...
                    if (BUS_NhaCungCap.Intance.themNCC(ncc))
                    {
        ...
        private void btnSuaNCC_Click(object sender, EventArgs e)
        {
            if (dgvNCC.SelectedCells.Count > 0)
            {
                ...
                ncc.Email = txtEmailNCC.Text;
                ncc.SDT = int.Parse(txtDienThoaiNCC.Text);
                if (BUS_NhaCungCap.Intance.suaNCC(ncc))
                {
        ...
        private void btnXoaNCC_Click(object sender, EventArgs e)
        {
            ...
            {
                    BUS_NhaCungCap.Intance.xoaNCC(txtMaNCC.Text);
        ...
        {
            dgvNCC.DataSource = BUS_NhaCungCap.Intance.TimKiemNCC(txtTimKiemNCC.Text);
        }
